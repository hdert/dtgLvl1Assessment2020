...
"use strict";
/* Declare frequently used HTML DOM elements so as to not slow down the DOM element finder */ const WRAPPER = document.getElementById("wrapper");
...
async function exitLoadingScreen() {
    ...
    document.getElementById("loadingScreen").style.animation = "slideOutBottom 0.9s 1.5s forwards ease-out";
    await sleep(1450);
    WRAPPER.style.overflowY = "auto";
/* This makes it so that the scrollbar doesn't show until the loading animation is near complete. */ /* A function that exits the loading screen by making each element transparent then making
...
setting a transition time in the css. */ /* Scroll functions */ function goToTop() {
    WRAPPER.scrollTop = 0;
...
or scrolled down since the last function call */ function scrollIndicatorUpdate() {
    PROGRESSBAR.style.width = WRAPPER.scrollTop / (WRAPPER.scrollHeight - document.documentElement.clientHeight) * 100 + "%";
}
/* This function updates the progressBar at the bottom of the navbar everytime the user
scrolls proportional to where they are on the page in the x-axis at the moment. */ function backToTopButtonUpdate() {
    if (WRAPPER.scrollTop > 250) document.getElementById("backToTopButton").style.animation = "fadeIn ease 0.3s";
    else document.getElementById("backToTopButton").style.animation = "fadeOut ease 0.3s forwards";
}
/* This function checks if the user has scrolled past a certain point, if they have
it shows the backToTopButton and if they haven't it hides it. */ WRAPPER.onscroll = function() {
    backToTopButtonUpdate();
    navbarHide();
    scrollIndicatorUpdate();
};

//# sourceMappingURL=shinyShinyDistros.c3942fc7.js.map
